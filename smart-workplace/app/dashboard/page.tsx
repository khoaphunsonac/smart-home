...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Get user's rooms
    const rooms = mockData.rooms.filter((room) => room.user_id === userData.id)
    setUserRooms(rooms)
  }, [router])
  ...
  const getEnvironmentData = (roomId: number) => {
    return mockData.environmentData.find((data) => data.room_id === roomId)
  }

  const getDeviceCount = (roomId: number) => {
    return mockData.devices.filter((device) => device.room_id === roomId).length
  }
